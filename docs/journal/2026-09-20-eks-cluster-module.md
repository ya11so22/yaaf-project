...
- **The node group is not emulated as N nodes.** Desired size 2 still yields a single k3s node
  ...
  `yaaf-floci-default`.
- In Floci's default in-memory mode, recreating it wipes its IAM, so the kubectl IAM user/key had to be recreated each time. Made-up keys are rejected too: the key must exist in Floci's IAM, and `create-access-key` can't take a chosen ID. Fix: `FLOCI_STORAGE_MODE=persistent` on a named volume in `compose.yaml`. Tested: an IAM user + key (and an S3 bucket) survive both `docker compose restart` and `down`/`up`. Also tested after `tofu apply`: `docker compose restart floci` leaves `tofu plan` at "No changes" (EC2/EKS metadata persists, so the OpenTofu state stays valid), and Floci recreates the k3s container itself on startup ("a surviving data volume is reused") within ~15s, after which `kubectl` works with the same key (node reports v1.36.4). The k3s data volume is reused, so cluster objects (system pods) survive, but the new container gets a new hostname and registers as a new node: the old node lingers as a `NotReady` ghost after every Floci restart (`kubectl get nodes` showed two). Clean up with `docker exec floci-eks-yaaf-floci kubectl delete node <old-name>` (set `MSYS_NO_PATHCONV=1` in Git Bash). Persistence is a local-dev choice only; CI would still get a fresh Floci.
- `kubectl-setup.ps1` rotates the key on every run, which invalidates any other profile/kubeconfig holding the old key; also its first version failed when the IAM user didn't exist yet (`aws iam get-user` stderr is fatal in Windows PowerShell 5.1 under `$ErrorActionPreference = "Stop"`).
- Watch
  for a trailing `
