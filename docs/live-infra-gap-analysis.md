...
| **Ingress, DNS, TLS** | None. Tools and the app are reached with `kubectl port-forward`. | Load balancer, DNS, certificates. | Not in Phase 1. Traefik arrives in Phase 2, with per-PR environments, which need host-based routing; TLS and real DNS stay out until real AWS. |
| **Observability** | None. | Metrics, logs, at least one alert. | Metrics stack, SLO-based alerts and an incident drill in Phase 2, together with DORA metrics. Tracing is an optional stretch. |
