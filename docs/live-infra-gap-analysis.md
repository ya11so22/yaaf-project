...
| **Ingress, DNS, TLS** | A Floci ALB in front of Traefik, with host-based routing on `*.localhost:8080` (ADR-0016). No TLS. | Load balancer, DNS, certificates. | TLS (ACM) and real DNS (Route 53) wait for real AWS. Per-PR environments get one host each in Phase 2. |
